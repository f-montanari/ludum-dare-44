...
{  
    ...
    void Start()
    {        
        playerEntity = GameObject.FindGameObjectWithTag("Player").GetComponent<NavEntityBehaviour>();        
        myEntity = GetComponent<NavEntityBehaviour>();
        myEntity.currentTarget = playerEntity;
    ...
    private void FollowingUpdate()
    {
        ...
        {
            // We're not, keep chasing target unless we find an enemy.
            if(myEntity.currentEnemy == null)
            {
                myEntity.currentEnemy = CheckForEntity("Enemy");
                if (myEntity.currentEnemy != null)
                {
